...
By default, `Flagfit.create()` creates a service implementation at runtime using `java.lang.reflect.Proxy`.
The **flagfit-ksp** module generates the implementation at compile time with [Kotlin Symbol Processing (KSP)](https://kotlinlang.org/docs/ksp-overview.html), so no reflection is used when evaluating flags.
...
dependencies {
    ...
    implementation 'com.github.abema.flagfit:flagfit:1.1.8'
    // Flagfit ksp
    ksp 'com.github.abema.flagfit:flagfit-ksp:1.1.8'
}
```

For each interface that declares flag functions, a `<InterfaceName>Impl` class and a `create<InterfaceName>()` extension function are generated:

```kotlin
// Generated by flagfit-ksp from the FlagService interface
val flagService: FlagService = flagfit.createFlagService()
val awesomeFeatureEnabled = flagService.awesomeFeatureEnabled()
```

The generated implementation behaves the same as the dynamic proxy created by `Flagfit.create()`, with the following differences:

- Flags are resolved without reflection. The resolution of each flag function is cached lazily.
- `suspend` functions don't need `@SuspendReturnType` because the return type is known at compile time.
- Misconfigurations such as a flag function with parameters or a `@BooleanFlag` function that does not return `Boolean` are reported as compile errors instead of runtime errors.
